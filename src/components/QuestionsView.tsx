...
import { 
  ...
  TIP_STRIP_PART3,
  REVIEW_QUESTIONS,
  ACTION_PLAN_FLOW_CHART,
  ACTION_PLAN_SHORT_ANSWER,
  ACTION_PLAN_TFNG,
  STRATEGY_EXTRACTS,
...
export const QuestionsView: React.FC<QuestionsViewProps> = ({
  ...
  const [isActionPlanFlowChartOpen, setIsActionPlanFlowChartOpen] = useState(false);
  const [isActionPlanShortAnswerOpen, setIsActionPlanShortAnswerOpen] = useState(false);
  ...
  // Active question filter tab (all, part1: 1-5 Flow Chart, part2: 6-9 Short Answer, part3: 10-13 TFNG)
    ...
    >
      ...
      <div className="p-4 sm:p-6 overflow-y-auto space-y-8 flex-1">
        ...
        {mode === 'practice' && (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-purple-950 uppercase tracking-wide">
                  Strategy Training: True / False / Not Given & Flow Chart (Extracts A–F)
                </span>
              </div>
                        ...
                        <div className="pt-2 border-t border-purple-100/70">
                          <div className="flex items-center gap-1.5 mb-2">
                            {(['TRUE', 'FALSE', 'NOT GIVEN'] as const).map((opt) => (
                              <button
                                key={opt}
                                id={`extract-${ext.id}-${opt.toLowerCase().replace(/\s+/g, '-')}`}
                                onClick={() =>
                                  setExtractUserAnswers((prev) => ({
                                    ...prev,
                                    [ext.id]: opt,
                                  }))
                                }
                                className={`flex-1 py-1 px-1 rounded text-[10px] font-bold border transition-colors cursor-pointer ${
                                  userChoice === opt
                          ...
                          <div className="flex items-center justify-between">
                            <button
                              id={`reveal-extract-${ext.id}`}
                              onClick={() =>
                                setExtractRevealed((prev) => ({
                                  ...prev,
                                  [ext.id]: !prev[ext.id],
                                }))
                              }
                            ...
                            >
                              {isRevealed ? 'Hide Analysis' : 'Verify & Show Analysis'}
                            </button>

                            {userChoice && isRevealed && (
                              <span
                                className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                                  isCorrect ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                                }`}
                              >
                                {isCorrect ? 'Correct!' : `Answer: ${ext.officialAnswer}`}
                              </span>
                            )}
                          </div>

                          {isRevealed && (
                            <div className="mt-2 p-2 rounded bg-white border border-purple-100 space-y-1 text-[11px]">
          ...
          {/* Error Analysis Module: Reasons A-F */}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-rose-950 uppercase tracking-wide">
                  Error Analysis: Diagnosing Typical Candidate Mistakes
                </span>
              </div>
              <span className="text-xs font-semibold text-rose-700 flex items-center gap-1">
                {isErrorAnalysisOpen ? 'Hide' : 'Show 6 Error Scenarios'}
                {isErrorAnalysisOpen ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
              </span>
            </button>

            {isErrorAnalysisOpen && (
              <div className="px-4 py-3 border-t border-rose-100 bg-white space-y-4 text-xs">
                <p className="text-slate-600">
                  Match each incorrect candidate answer with the exact diagnostic reason (Reasons A–F).
                </p>

                {/* Legend of Reasons A-F */}
                <div className="p-2.5 rounded-lg bg-rose-50/50 border border-rose-100 grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px]">
                  {ERROR_ANALYSIS_REASONS.map((r) => (
                    <div key={r.id} className="flex items-start gap-1.5 text-slate-700">
                      <span className="shrink-0 w-4 h-4 rounded bg-rose-200 text-rose-900 font-bold text-[10px] flex items-center justify-center">
                        {r.id}
                      </span>
                      <span>
                        <strong>Reason {r.id}:</strong> {r.text}
                        {explanationLanguage !== 'en' && r.textVi && (
                      ...
                      <div key={item.id} className="p-3 rounded-lg border border-slate-200 bg-slate-50/50 space-y-2">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <span className="text-xs font-bold text-slate-800">
                            Scenario {item.id}: {item.questionPrompt}
                          </span>
                        ...
                        <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-200">
                          <span className="text-[11px] font-medium text-slate-600">Select diagnosis:</span>
                          <div className="flex items-center gap-1">
                            {['A', 'B', 'C', 'D', 'E', 'F'].map((reasonLetter) => (
                              <button
                                key={reasonLetter}
                                id={`error-item-${item.id}-reason-${reasonLetter}`}
                                onClick={() =>
                                  setErrorAnalysisMatches((prev) => ({
                                    ...prev,
                                    [item.id]: reasonLetter,
                                  }))
                                }
                                className={`w-6 h-6 rounded text-[11px] font-bold border transition-colors cursor-pointer ${
                                  userMatch === reasonLetter
                          ...
                          <button
                            ...
                            className="ml-auto text-[11px] text-rose-700 hover:text-rose-900 font-semibold cursor-pointer"
                          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {/* Action Plan 1: Flow Chart */}
            <div className="border border-blue-200 bg-blue-50/70 rounded-xl overflow-hidden shadow-2xs">
              <button
                id="toggle-action-plan-flowchart-btn"
                onClick={() => setIsActionPlanFlowChartOpen(!isActionPlanFlowChartOpen)}
              ...
              >
                ...
                {isActionPlanFlowChartOpen ? <ChevronUp className="w-3.5 h-3.5 text-blue-700" /> : <ChevronDown className="w-3.5 h-3.5 text-blue-700" />}
              </button>

              {isActionPlanFlowChartOpen && (
                <div className="px-3 py-2 border-t border-blue-100 bg-white space-y-2 text-[11px]">
                  {ACTION_PLAN_FLOW_CHART.items.map((item) => (
            ...
            {/* Action Plan 2: Short Answer */}
            <div className="border border-amber-200 bg-amber-50/70 rounded-xl overflow-hidden shadow-2xs">
              <button
                id="toggle-action-plan-short-btn"
                onClick={() => setIsActionPlanShortAnswerOpen(!isActionPlanShortAnswerOpen)}
                className="w-full px-3 py-2 flex items-center justify-between text-left hover:bg-amber-100/50 transition-colors cursor-pointer"
              >
                <div className="flex items-center gap-1.5">
                ...
                {isActionPlanShortAnswerOpen ? <ChevronUp className="w-3.5 h-3.5 text-amber-700" /> : <ChevronDown className="w-3.5 h-3.5 text-amber-700" />}
              </button>

              {isActionPlanShortAnswerOpen && (
                <div className="px-3 py-2 border-t border-amber-100 bg-white space-y-2 text-[11px]">
                  {ACTION_PLAN_SHORT_ANSWER.items.map((item) => (
                    <div key={item.number} className="p-2 rounded bg-amber-50/40 border border-amber-100">
                      <p className="font-semibold text-slate-900">{item.question}</p>
                      <p className="mt-0.5 text-amber-950 font-medium">👉 {item.answer}</p>
                    </div>
                  ))}
                </div>
              )}
        ...
        {(activeFilter === 'all' || activeFilter === 'part1') && (
          <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 transition-all">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2 text-blue-900 font-bold text-sm">
                    ...
                    <span>IELTS Tip & Strategy: Flow Chart Completion</span>
                  </div>
                  <button
            ...
            {/* Interactive Visual Flow Chart Component */}
            <TableCompletionView
                      ...
                      <div className="mt-3 p-3.5 rounded-lg bg-blue-50/60 border border-blue-200 text-xs text-slate-800 space-y-2.5 animate-in fade-in">
                        ...
                        {q.distractorAnalysis && (
                          <div className="p-2 rounded bg-amber-50/60 border border-amber-200 text-[11px] text-amber-950">
                            <span className="font-bold block mb-0.5 text-amber-900">⚠️ Common Pitfall:</span>
                            <p>{q.distractorAnalysis}</p>
                            {explanationLanguage !== 'en' && q.distractorAnalysisVi && (
                              <p className="italic text-amber-900 mt-1">🇻🇳 {q.distractorAnalysisVi}</p>
                            )}
                          </div>
                        )}
                      </div>
        ...
        {/* ========================================================= */}
        {/* PART 2: Questions 6–9 (Short Answer Questions)            */}
        {/* ========================================================= */}
        {(activeFilter === 'all' || activeFilter === 'part2') && (
          <div className="space-y-6 pt-2">
            <div className="bg-[#0F172A] text-white p-4 sm:p-5 rounded-xl shadow-xs border border-slate-800">
              ...
              <h3 className="text-base sm:text-lg font-bold mt-2">
                {renderHighlightedText('Answer the questions below.')}
              </h3>
              <p className="text-xs text-slate-300 mt-1 font-medium">
            ...
            {mode === 'practice' && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 transition-all">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2 text-blue-900 font-bold text-sm">
                    <Lightbulb className="w-4 h-4 text-blue-600" />
                    <span>IELTS Tip & Strategy: Short Answer Questions</span>
                  </div>
                  <button
        ...
        {/* ========================================================= */}
        {/* PART 3: Questions 10–13 (True / False / Not Given)        */}
        {/* ========================================================= */}
